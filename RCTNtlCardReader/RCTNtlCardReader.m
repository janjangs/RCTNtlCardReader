    ...
    for (i = 0; i < [istapdu count]; i++) {
        ...
        if (iRet != 0) {
            ...
            if(i == 0){
                ...
                NSLog (@"resp = %s", resp);
                NSLog (@"resplen = %i", resplen);
                [RevData appendBytes:resp length:resplen];
                NSLog (@"RevData = %@", RevData);
                NSLog (@"RevData length = %lu", (unsigned long)RevData.length);
                capdulen = (unsigned int)[RevData length];
                NSLog (@"capdulen = %i", capdulen);
                
                NSString *str = [RevData description];
                NSLog (@"str = %@", str);
                NSLog (@"str length = %lu", (unsigned long)str.length);
                str = [NSString stringWithFormat: @"%@%@", @"00C00000", [str substringWithRange:NSMakeRange(3, 2)]];
                NSData *readapduData =[self hexFromString:str];
